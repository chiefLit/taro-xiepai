...
export default class ExpressInfo extends Component {
  ...
  async getExpressCompanyList() {
    let data = await getExpressCompanyList(null)
    if (data.code !== 1) {
      Taro.showToast({
        title: data.message,
        icon: 'none'
  ...
  async addExpressInfo() {
    ...
    let data = await orderApi.addExpressInfo({
      ...
      <View className='express-info-wrapper'>
        ...
        <View className='module-list'>
          ...
          <View className='modue-item'>
            ...
            <View className='value'>
              <Input placeholder='请填写单号' value={toStoreExpressNumber} type='text'
                onInput={(e) => {
                  this.setState({
        ...
        <AtActionSheet isOpened={showExpressCompanyList} cancelText='取消'>
          {
            expressCompanyList.map((ele) => {
              return (
                <AtActionSheetItem key={ele.id}
                  onClick={() => {
                    this.setState({
